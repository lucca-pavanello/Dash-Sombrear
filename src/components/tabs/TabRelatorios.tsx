...
export default function TabRelatorios() {
  ...
  const porCanal = useMemo(() => {
    ...
    return [...mapa.entries()]
      ...
      .map(([id, v]) => ({ id, ...v, ticket: v.fechamentos > 0 ? v.faturamento / v.fechamentos : 0 }))
  ...
  async function exportarPdf() {
    ...
    try {
      ...
      autoTable(doc, {
        startY: 32,
        head: [['Canal', 'Leads', 'Orçamentos', 'Fechamentos', 'Faturamento', 'Ticket médio']],
        body: porCanal.map(c => [
    ...
    <>
      ...
      <div className="kpi-cascade mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          <div key={k.rotulo} className={cn('rounded-xl border bg-card p-4 shadow-sm',
