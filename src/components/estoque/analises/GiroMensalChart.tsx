...
function CustomTooltip({ active, payload, label }: TooltipProps) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card px-3 py-2.5 shadow-md text-xs space-y-1.5 min-w-[160px]">
      <p className="text-sm font-medium text-foreground">{label}</p>
      <hr className="border-border/60" />
      {giroR && (
        <p>
          <span className="text-muted-foreground">Giro R$: </span>
          <span className="font-medium text-orange-700 dark:text-orange-400">{giroR.value.toFixed(2)}×</span>
        </p>
      )}
      {giroU && (
        <p>
          <span className="text-muted-foreground">Giro un.: </span>
          <span className="font-medium text-foreground/80">{giroU.value.toFixed(2)}×</span>
        </p>
      )}
      <p>
        <span className="text-muted-foreground">Vendas: </span>
        <span className="text-muted-foreground">{fmtBRL(d?.vendas_reais ?? 0)}</span>
      </p>
    </div>
  )
}
